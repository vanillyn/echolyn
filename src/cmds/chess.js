...
export default {
	data: new SlashCommandBuilder()
		...
		.addSubcommand(sub =>
			sub
				...
				.addUserOption(opt =>
					opt.setName('opponent').setDescription('Player to challenge').setRequired(false)
				)
				.addStringOption(opt =>
					opt
						.setName('variant')
		...
		.addSubcommand(sub =>
			sub
				...
				.addUserOption(opt =>
					opt
						.setName('target')
						.setDescription('Player the server will face')
						.setRequired(true)
				)
				.addStringOption(opt =>
					opt
		...
		.addSubcommand(sub =>
			sub
				...
				.addIntegerOption(opt =>
					opt
						.setName('difficulty')
						.setDescription('Engine strength (800-2800)')
						.setMinValue(800)
						.setMaxValue(2800)
				)
				.addStringOption(opt =>
		...
		.addSubcommand(sub =>
			sub
				...
				.addUserOption(opt =>
					opt
						.setName('opponent')
						.setDescription('Player to play against')
						.setRequired(true)
				)
				.addIntegerOption(opt =>
					opt
						.setName('hours')
						.setDescription('Hours per move (1-72)')
						.setMinValue(1)
						.setMaxValue(72)
				)
		)
		.addSubcommand(sub => sub.setName('resign').setDescription('Resign current game'))
		.addSubcommand(sub =>
			sub
				.setName('vote')
				...
				.addStringOption(opt =>
					opt
						.setName('move')
						.setDescription('Your move vote (e.g. e4, Nf3)')
						.setRequired(true)
				)
		),
	...
	async handleChallenge(interaction) {
		...
		if (opponent) {
			if (opponent.id === interaction.user.id) {
				return interaction.reply({
					content: 'Cannot challenge yourself',
					flags: MessageFlags.Ephemeral,
				});
			}
			if (opponent.bot) {
				return interaction.reply({
					content: 'Cannot challenge bots',
					flags: MessageFlags.Ephemeral,
				});
			}
			...
			const components = [
				new TextDisplayBuilder().setContent(
					`# ${VARIANTS[variant]} Challenge\n\n${interaction.user} is looking for an opponent!`
				),
	...
	async handleServerVs(interaction) {
		...
		if (target.bot) {
			return interaction.reply({
				content: 'Cannot play server vs bot',
				flags: MessageFlags.Ephemeral,
			});
		}
	...
	async handleCorrespondence(interaction) {
		...
		if (opponent.id === interaction.user.id) {
			return interaction.reply({
				content: 'Cannot play correspondence with yourself',
				flags: MessageFlags.Ephemeral,
			});
		}
		...
		if (existingGame) {
			return interaction.reply({
				content: 'You already have a correspondence game with this player',
				flags: MessageFlags.Ephemeral,
			});
		}
		...
		try {
			await opponent.send({
				components: [
					new TextDisplayBuilder().setContent(
						`# Correspondence Chess Game Started\n\n${interaction.user} has started a correspondence game with you!\n\n**Time per move:** ${hours} hours\n\nUse \`/chess move\` to make moves.`
					),
				],
				flags: MessageFlags.IsComponentsV2,
			});
			...
			return interaction.reply({
				content: 'Failed to start correspondence game. Make sure both players allow DMs.',
				flags: MessageFlags.Ephemeral,
			});
		}

		return interaction.reply({
			content: `Correspondence game started with ${opponent}! Check your DMs.`,
			flags: MessageFlags.Ephemeral,
		});
	},
	...
	async handleResign(interaction) {
		...
		return interaction.reply({
			content: `${await game.getPlayerName(0, client)} vs ${await game.getPlayerName(
				1
	...
	async handleVote(interaction) {
		...
		if (!game || game.variant !== 'servervs') {
			return interaction.reply({
				content: 'No server vs player game active in this channel',
				flags: MessageFlags.Ephemeral,
			});
		}
		...
		if (!game.isServerTurn()) {
			return interaction.reply({
				content: "Not the server's turn to move",
				flags: MessageFlags.Ephemeral,
			});
		}
		...
		if (result && result.voted) {
			...
			const voteList = votes
				.map(([userId, userMove]) => `<@${userId}>: ${userMove}`)
				.join('\n');
	...
	async startGame(interaction, players, options) {
		...
		const { components, attachment } = await this.buildGameMessage(game, interaction.user.id);
	...
	async buildGameMessage(game, requestUserId = null) {
		...
		const configUserId =
			...
			!['stockfish', 'random', 'server', 'both'].includes(currentPlayerId)
				? currentPlayerId
				: requestUserId;
    const players = {
      white: await game.getPlayerName(0, client),
      black: await game.getPlayerName(1, client),
    }
		...
		const viewer = new ContainerBuilder()
			.addTextDisplayComponents(textDisplay =>
				textDisplay.setContent(
					`# ${players.white} vs ${players.black} | ${
						VARIANTS[game.variant] || game.variant
					}`
				)
			)
			...
			.addMediaGalleryComponents(MediaGalleryBuilder =>
				MediaGalleryBuilder.addItems(MediaGalleryItemBuilder =>
					MediaGalleryItemBuilder.setDescription('A chess game being played.').setURL(
		...
		if (!state.gameOver) {
			const actionComponents = this.getGameComponents(game, state);
			if (actionComponents.length > 0) {
				viewer.addActionRowComponents(...actionComponents);
			}
		}

		const gameDescription = this.getGameDescription(game, state);
		if (gameDescription) {
      viewer.addSeparatorComponents(SeparatorBuilder => SeparatorBuilder.setSpacing(SeparatorSpacingSize.Small))
      viewer.addTextDisplayComponents( textDisplay => textDisplay.setContent(gameDescription))
		}
	...
	getGameComponents(game, state) {
		...
		if (game.variant === 'realtime') {
			components.push(
				new ActionRowBuilder().addComponents(
					new ButtonBuilder()
						.setCustomId('realtime_move')
						.setLabel('Queue Move')
						.setStyle(ButtonStyle.Primary),
					new ButtonBuilder()
						.setCustomId('flip_board')
						.setLabel('Flip Board')
			...
			components.push(
				new ActionRowBuilder().addComponents(
					new ButtonBuilder()
						.setCustomId('blitz_instant')
						.setLabel('⚡ Instant')
						.setStyle(ButtonStyle.Primary),
					new ButtonBuilder()
						.setCustomId('blitz_think')
						.setLabel('🤔 Think')
						.setStyle(ButtonStyle.Secondary),
					new ButtonBuilder()
						.setCustomId('blitz_normal')
						.setLabel('Normal')
						.setStyle(ButtonStyle.Secondary)
				)
			);
		} else {
			const StandardButtons = new ActionRowBuilder().addComponents(
				new ButtonBuilder()
					.setCustomId('make_move')
					.setLabel('Make Move')
					.setStyle(ButtonStyle.Primary),
				new ButtonBuilder()
					.setCustomId('flip_board')
			...
			components.push(StandardButtons);
		}
	...
	setupGameCollector(message, game) {
		...
		collector.on('collect', async i => {
			...
			if (
				i.customId === 'make_move' ||
				i.customId === 'realtime_move' ||
				i.customId === 'vote_move'
			) {
			...
			if (i.customId.startsWith('blitz_')) {
				const reactionType =
					i.customId === 'blitz_instant'
						? '⚡'
						: i.customId === 'blitz_think'
						? '🤔'
						: null;
			...
			if (i.customId === 'resign') {
				if (!game.isInGame(i.user.id)) {
					return i.reply({
						components: [
							new TextDisplayBuilder().setContent('You are not in this game'),
						],
						flags: MessageFlags.Ephemeral,
					});
				}
	...
	async updateGameMessage(interaction, game, requestUserId) {
		const { components, attachment } = await this.buildGameMessage(game, requestUserId);
