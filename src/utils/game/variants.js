	...
	movesConflict(move1, move2) {
		try {
			...
			if (typeof move1 === 'string') {
				parsedMove1 =
					move1.length === 4 || move1.length === 5
						? parseUci(move1)
						: testPos1.parseSan(move1);
			} else {
				parsedMove1 = move1;
			}
			...
			if (typeof move2 === 'string') {
				parsedMove2 =
					move2.length === 4 || move2.length === 5
						? parseUci(move2)
						: testPos2.parseSan(move2);
			} else {
				parsedMove2 = move2;
			}
...
export class VariantGame {
	...
	getGameState() {
		...
		if (this.variant === 'servervs') {
			...
			baseState.timeRemaining = this.voteEndTime
				? Math.max(0, this.voteEndTime - Date.now())
				: 0;
		}
	...
	getPgn() {
		...
		} else if (this.chess.isStalemate() || this.chess.isInsufficientMaterial()) {
			pgn += '1/2-1/2';
		}
	...
	resign(userId) {
		...
		if (
			this.rated &&
			this.gameType === 'pvp' &&
			typeof this.players[0] === 'string' &&
			typeof this.players[1] === 'string'
		) {
			database
				.updateEcholynRating(
					this.players[0],
	...
	async finishGame() {
		...
		if (result) {
			const ratingChanges = await database.updateEcholynRating(
				player1Id,
				player2Id,
				result,
				this.getPgn()
